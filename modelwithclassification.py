...
class CalibratedSelectiveLoss(nn.Module):
    ...
    Total = α * L_selective + (1-α) * L_auxiliary + β * L_calibration
...
def evaluate_hypothesis(model, test_loader, device, method='selectivenet'):
    ...
    if valid_results:
        ...
        if best['acc_boost'] > 0.02:  # >2% improvement
            print(f"\n✅ HYPOTHESIS SUPPORTED: Significant accuracy boost "
                  f"({best['acc_boost']*100:.1f}%) with low rejection ({best['rejected_pct']:.1f}%)")
        else:
            print(f"\n⚠️  Marginal improvement. Consider adjusting coverage target or training longer.")
...
def demonstrate_medical_use_cases(model, test_loader, device):
    ...
    print("\n📋 USE CASE 1: AUTOMATED TRIAGE SYSTEM")
    print("-" * 50)
    ...
    print(f"  🤖 Auto-diagnosed: {auto_diagnose} patients ({auto_diagnose/len(images)*100:.1f}%)")
    print(f"  👨‍⚕️ Flagged for specialist: {specialist_review} patients ({specialist_review/len(images)*100:.1f}%)")

    # Accuracy on auto-diagnosed
    if accepted.sum() > 0:
        auto_acc = (preds[accepted] == labels[accepted].view(-1)).float().mean().item()
        print(f"  ✅ Auto-diagnosis accuracy: {auto_acc*100:.2f}%")
    ...
    print(f"\n  🟢 High confidence (≥0.8):   {high_conf.sum().item()} cases - Auto-process")
    print(f"  🟡 Medium confidence (0.5-0.8): {medium_conf.sum().item()} cases - Quick review")
    print(f"  🔴 Low confidence (<0.5):    {low_conf.sum().item()} cases - Full specialist review")

    # === USE CASE 4: Quality Control ===
    print("\n🔍 USE CASE 4: QUALITY CONTROL / ANOMALY DETECTION")
    print("-" * 50)
...
MODEL_PATH = "./models/resnet-preloaded-weights-swapped-last-layer-full-model-trained.pth"
...
if __name__ == "__main__":
    ...
    # ========================================================================
    # Load the pretrained model (handle both TorchScript and regular state_dict)
    # ========================================================================
    print("Loading model...")
    try:
        ...
        print(f"TorchScript loading failed ({e}), trying regular state_dict...")
        try:
            # Fallback to regular state_dict with weights_only=False for TorchScript compatibility
            scripted_state = torch.load(MODEL_PATH, map_location=device, weights_only=False)
            print("Loaded as regular state_dict")
        except Exception as e2:
            raise RuntimeError(f"Failed to load model with both methods. TorchScript error: {e}, State_dict error: {e2}")
    
    # Check the fc layer structure
    print("Model fc keys:", [k for k in scripted_state.keys() if 'fc' in k])
